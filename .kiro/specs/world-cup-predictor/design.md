...
A floating "scroll to top" button (`src/components/scroll-to-top-button.tsx`) is rendered on all pages that use auto-scroll. It appears (fixed, bottom-right) after scrolling 400px down, using smooth `window.scrollTo` on click. This allows users to quickly return to the top of long match lists after being auto-scrolled to mid-page.
