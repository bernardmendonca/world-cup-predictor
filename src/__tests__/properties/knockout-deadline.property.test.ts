...
describe("Property 7: Knockout Stage Prediction Deadline", () => {
  const TWO_HOURS_MS = 2 * 60 * 60 * 1000;

  // Generate arbitrary kickoff times for knockout stage
  const kickoffArb = fc.date({
    min: new Date("2026-07-01T00:00:00Z"),
    max: new Date("2026-07-20T00:00:00Z"),
  });
  ...
  it("deadline is exactly 2 hours before kickoff (same rule as group stage)", () => {
    fc.assert(
